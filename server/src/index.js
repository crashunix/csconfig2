...
createServer = () => {
  ...
  io.on('connection', function (socket) {
    ...
    socket.on('say', function (say) {
      ...
      fs.writeFile(`${config.gamePath}\\csgo\\cfg\\csconfig.cfg`, `say "${say}"`, (err) => {
        if (err) throw err;
        robot.keyTap(',');
      });
    });
    ...
    socket.on('command', function (command) {
      console.log('Received bind:', command);
      fs.writeFile(`${config.gamePath}\\csgo\\cfg\\csconfig.cfg`, `${command}`, (err) => {
        if (err) throw err;
        console.log(`${command}`);
        robot.keyTap(',');
      });
